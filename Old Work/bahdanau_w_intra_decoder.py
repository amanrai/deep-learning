...
#does not implement max(2j, 2j+1) selection
#uses intra-decoder attention from https://arxiv.org/pdf/1705.04304.pdf
#-Aman Rai
...
class AttentionDecoder(Recurrent):
    ...
    def build(self, input_shape):
        ...
        self.W_intra = self.add_weight(shape=(self.hidden_units, self.hidden_units),
            name="w_intra", initializer=self.kernel_initializer)
        
        self.Cdo = self.add_weight(shape=(self.hidden_units, self.output_dim), 
        name="cdo", initializer=self.kernel_initializer)
    ...
    def step(self, x,  states):
        ...
        all_states = K.permute_dimensions(K.stack(self.output_states), (1,0,2))
        ...
        st_ = K.expand_dims(st, axis=1) #(batch_size,1,hidden_units)
        st_intra = K.dot(st_, self.W_intra) #(batch_size, 1, hidden_units)
        et_intra = K.batch_dot(all_states,st_intra, axes=2) #(batch_size, t-1, hidden_units)
        alpha = K.softmax(et_intra) #(batch_size, t-1, 1)
        cdt = K.sum(all_states*alpha, axis=1) #(batch_size, 1, hidden_units)
        
        #calculate the output
        ti = K.dot(s_p, self.Uo) + K.dot(y_p, self.Wo) + K.dot(cti, self.Co) + K.dot(cdt, self.Cdo) + self.bo 
        
        #append this state to the previous states
        self.output_states.append(st)
